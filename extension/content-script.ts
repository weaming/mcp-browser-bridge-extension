...
function doPress(key: string): void {
  ...
  // 合成键盘事件不会触发浏览器原生表单提交;Enter 时用 requestSubmit 兜底
  if (key === "Enter") {
...
async function execute(action: Action): Promise<ContentResponse> {
  ...
  try {
    ...
    if (el === undefined) {
      return { kind: "execute-result", seq: 0, ok: false, code: "stale-ref", detail: `ref ${action.ref} 不存在` };
    }
